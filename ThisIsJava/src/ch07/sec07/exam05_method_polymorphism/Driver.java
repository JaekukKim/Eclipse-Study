...
public class Driver {
	public void drive(Vehicle vehicle) { // vehicle객체만 들어올 수 있는게 아니라 vehicle객체를 상속받은 자식 객체들도 들어올 수 있다.
					//여기 위에 들어오는 객체에 따라서 그 객체에서 오버라이딩 된 메소드가 실행이 된다.
		vehicle.run(); // 자식 객체 bus,taxi에 상속을 해 주었기 때문에 재정의된 run메소드가 실행이 된다.
